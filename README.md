...
Biometric Integration is a Frappe app for syncing Hikvision terminal events into employee attendance logs.

## Feature support

- Device connectivity test from settings.
- Attendance sync for a selected date/time range.
- Scheduled attendance sync (cron) for recent days.
- Digest-auth based API communication with Hikvision ISAPI.
- Automatic creation and update of daily attendance logs.
- Duplicate punch detection during sync.
- Auto fetch employee names from device during sync.
- Update employee name on device from the app.
- Manual punch entry linked to Employee records.
- Manual punch edit flow (replace old date/time with new date/time).
- Manual punch delete flow that also removes linked punch row from attendance logs.
- Bulk rebuild of attendance logs from all saved manual punches.
- Punch type tracking (Auto and Manual).
- Leave log doctype for employee leave-time records.
- Workspace shortcuts for reports and core doctypes.

## Reports

- Biometric Daily Report
  - Shows punch timeline, total hours, and early-leave flag.
  - Includes absent active employees in output.
  - Highlights manual punches in report display.
- Biometric Monthly Report
  - Day-wise hours for each active employee.
  - Total hours in HH:MM and decimal format.
- Biometric Manual Punch Report (present in app but currently disabled).
...
- Employee records with `attendance_device_id` for mapping.

## Supported devices

This app targets Hikvision terminals that expose ISAPI access-control endpoints used by the sync flow. The repository also includes a validated model list from implementation testing:
